...
async def list_batch_pins(batch_id: str, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(Pin).options(selectinload(Pin.extraction), selectinload(Pin.listing_draft), selectinload(Pin.catalog_matches), selectinload(Pin.comps)).where(Pin.batch_id == batch_id)
    )
...
async def get_pin_detail(pin_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(Pin).options(selectinload(Pin.extraction), selectinload(Pin.listing_draft), selectinload(Pin.catalog_matches), selectinload(Pin.comps)).where(Pin.id == pin_id)
    )
...
def _pin_to_dict(pin: Pin) -> dict:
    ...
    for comp in pin.comps:
        data["comps"].append({"title": comp.title, "price": comp.price, "listing_type": comp.listing_type.value, "excluded": comp.excluded})
    return data
